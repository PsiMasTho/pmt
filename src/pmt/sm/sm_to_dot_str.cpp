...
auto sm_to_dot_str(pmt::sm::StateMachine const& state_machine_, FinalIdToStringFn const& final_id_to_str_, std::string state_machine_dotfile_skel_) -> std::string {
 ...
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "FINAL_ID_TABLE", make_final_id_table_string(state_machine_, final_id_to_str_));
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "TIMESTAMP", get_timestamp_str());
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "LAYOUT_DIRECTION", "LR");
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "ACCEPTING_NODE_SHAPE", "doublecircle");
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "ACCEPTING_NODE_COLOR", rgb_to_str(0, 0, 255));
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "ACCEPTING_NODES", accepting_nodes_replacement);
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "NONACCEPTING_NODE_SHAPE", "circle");
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "NONACCEPTING_NODE_COLOR", rgb_to_str(0, 0, 0));
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "EPSILON_EDGE_COLOR", rgb_to_str(0, 255, 0));
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "EPSILON_EDGES", epsilon_edges_replacement);
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "SYMBOL_EDGES", symbol_edges_replacement);
 skeleton_replacer->replace_label(state_machine_dotfile_skel_, "GRAPH_TITLE", "Terminal State Machine");

 return state_machine_dotfile_skel_;
}
