...
auto get_e_closure(StateMachine const& state_machine_, Locals& locals_, StateNrType state_nr_from_) -> IntervalSet<StateNrType> {
...
auto get_e_closure(StateMachine const& state_machine_, Locals& locals_, IntervalSet<StateNrType> const& state_nrs_from_) -> IntervalSet<StateNrType> {
 IntervalSet<StateNrType> ret;

 state_nrs_from_.for_each_key([&](StateNrType state_nr_from_) { ret.inplace_or(get_e_closure(state_machine_, locals_, state_nr_from_)); });

 return ret;
}

auto get_moves(StateMachine const& state_machine_, IntervalSet<StateNrType> const& state_nrs_from_, SymbolType symbol_) -> IntervalSet<StateNrType> {
 IntervalSet<StateNrType> ret;

 state_nrs_from_.for_each_key([&](StateNrType state_nr_from_) {
  State const& state_from = *state_machine_.get_state(state_nr_from_);
  if (std::optional<StateNrType> const state_nr_next = state_from.get_symbol_transition(symbol_); state_nr_next.has_value()) {
...
auto get_symbols(StateMachine const& state_machine_, IntervalSet<StateNrType> const& state_nrs_from_) -> IntervalSet<SymbolType> {
 IntervalSet<SymbolType> ret;

 state_nrs_from_.for_each_key([&](StateNrType state_nr_from_) { ret.inplace_or(state_machine_.get_state(state_nr_from_)->get_symbols()); });
...
auto sm_determinize(StateMachine const& state_machine_, StateNrType state_nr_from_) -> StateMachine {
 ...
 push_and_visit(locals, get_e_closure(state_machine_, locals, state_nr_from_));

 while (!locals._pending.empty()) {
  StateNrType const state_nr_cur = take(locals);

  // Set up the final ids
  locals._new_to_old[state_nr_cur].for_each_key([&](StateNrType state_nr_old_) { locals._output_state_machine.get_state(state_nr_cur)->add_final_ids(state_machine_.get_state(state_nr_old_)->get_final_ids()); });

  // Set up the transitions
  get_symbols(state_machine_, locals._new_to_old[state_nr_cur]).for_each_key([&](SymbolType symbol_) {
   StateNrType const state_nr_next = push_and_visit(locals, get_e_closure(state_machine_, locals, get_moves(state_machine_, locals._new_to_old[state_nr_cur], symbol_)));
