...
namespace {
struct BitsetHasher {
 auto operator()(Bitset const& bitset_) const -> size_t {
  size_t seed = Hash::Phi64;

  for (Bitset::ChunkType const chunk : bitset_.get_chunks()) {
   Hash::combine(chunk, seed);
  }
  return seed;
 }
};
...
auto get_initial_partitions(StateMachine const& state_machine_, std::span<StateNrType const> state_nrs_, StateNrBitsetMapping const& mapping_, StateNrType state_nr_sink_) -> BitsetSet {
 ...
 // <accepts, state_nrs>
 std::unordered_map<IntervalSet<FinalIdType>, Bitset> by_accepts;

 auto insert = [&](StateNrType state_nr_, IntervalSet<FinalIdType> const& accepts_) {
 ...
 for (StateNrType const& state_nr_ : state_nrs_) {
  State const& state = *state_machine_.get_state(state_nr_);
  IntervalSet<FinalIdType> const& accepts = state.get_final_ids();
...
auto rebuild_minimized_state_machine(StateMachine const& state_machine_, StateNrBitsetMapping const& mapping_, std::unordered_map<StateNrType, Bitset const*> const& equivalence_partition_mapping_, StateNrType state_nr_from_, StateNrType state_nr_sink_) -> StateMachine {
 ...
 while (!pending.empty()) {
  ...
  result.get_state(state_nr_cur)->add_final_ids(state_old.get_final_ids());
